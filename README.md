...
**Disable modal editing** (optional): Add to `.loki/init.lua`:
```lua
...
Define `loki.highlight_row(row_index, line_text, render_text, syntax_type, default_applied)` inside `.loki/init.lua` (or `~/.loki/init.lua`) to extend or replace syntax highlighting from Lua. Return `nil` to keep the built-in rules, or return a table of span descriptors to colour specific regions:
...
Style strings map to constants exposed under `loki.hl` (`match`, `string`, `keyword1`, etc.). To layer extra cues without discarding the defaults, omit `replace` and simply return `{ spans = {...} }`.
...
Original [kilo](https://github.com/antirez/kilo) editor by Salvatore Sanfilippo (antirez). Lua integration and enhancements added 2025
